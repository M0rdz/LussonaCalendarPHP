<?php
    ...
    switch($_GET["op"])
    {
        ...
        case 'listar':
            ...
            $data = Array();
            while ($reg = $rspta->fetch_object()) {
                $url = '../reportes/Facturaingreso.php?id=';
                $data[] = array(
                    "0"=> (
                        ($reg->estado=='Aceptado')?'<button class="btn btn-warning" onclick="mostrar('.$reg->idingreso.')"><i class="fa fa-eye"></i></button>'.
                        ...
                        '<a target="_blank" href="'.$url.$reg->idingreso.'">
